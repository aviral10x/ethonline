...
async function logProof(proof: string) {
  console.log(
    "✅ here is the proof in byte32, ready to paste into the UI...\n0x" + proof
  );
}
...
async function logPublicInputs(circuitName: CircuitName) {
  const p = path.join(
    __dirname,
    `../../../../../noir/circuits/${circuitName}/Verifier.toml`
  );
  ...
  console.log(
    "🔍 here is the publicInputs, ready to paste into the UI...\n" +
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  res
    .status(200)
    .json("Proof generation started, result will be logged to server console");
}
